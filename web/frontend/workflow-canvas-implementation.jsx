...
import {Decoration, EditorView, MatchDecorator, ViewPlugin, placeholder as editorPlaceholder} from '@codemirror/view';
...
import {resetWorkflowNodeForRun, workflowNodeExecutionDuration, workflowNodeLiveDuration} from './workflow-execution-timing.mjs';
import {clampInspectorPosition} from './workflow-inspector-layout.mjs';
import {trimTrailingEmptyLlmNodeMessages} from './workflow-llm-messages.mjs';
import {createTemplateVariablePattern} from './workflow-markdown.mjs';
import {
    Background,
    BaseEdge,
    Controls,
...
const LLM_MESSAGE_PLACEHOLDERS = {
    SYSTEM: '为对话提供高层指导(通过${变量名}引用上下文)',
    USER: '向模型提供指令、查询或任何基于文本的输入(通过${变量名}引用上下文)',
    ASSISTANT: '基于用户消息的模型回复(通过${变量名}引用上下文)',
};
...
const templateVariableMatcher = new MatchDecorator({
    regexp: createTemplateVariablePattern(),
    decoration: Decoration.mark({class: 'cm-template-variable'}),
});

const templateVariableHighlighter = ViewPlugin.fromClass(class {
    constructor(view) {
...
function MarkdownPromptEditor({
    ...
    return (
        <div
            className={`wf-markdown-editor${expanded ? ' is-expanded' : ''}${error ? ' is-invalid' : ''}`}
            ref={hostRef}
...
function serializableNode(node) {
    ...
    for (const key of ('status executionDurationMs executionStartedAt runHistory executionId savedAt isDirty temporaryRun nodeTestActive nodeTestId nodeTestStartedAt'.split(' '))) {
        delete data[key];
    }
    return {
        id: node.id,
...
function NodePicker({onSelect, compact = false, includeSystem = false, excludeType = null, excludeTypes = []}) {
    const excluded = new Set([excludeType, ...excludeTypes].filter(Boolean));
...
function ContextMenu({menu, canPaste, canReplace, unavailableSystemTypes, onAction, onAdd, onReplace}) {
    ...
    return (
        <div className="wf-context-menu" style={{left: menu.x, top: menu.y}} role="menu" data-testid="pane-context-menu">
            <div className={`wf-context-submenu-trigger ${submenuOpen ? 'is-open' : ''}`}>
                ...
                <div className="wf-context-submenu"><NodePicker onSelect={onAdd} includeSystem excludeTypes={unavailableSystemTypes} /></div>
            </div>
...
const Inspector = React.memo(function Inspector({
    ...
    const [expandedLlmMessageId, setExpandedLlmMessageId] = useState(null);
    ...
    const expandedLlmTriggerRef = useRef(null);
    ...
    useEffect(() => {
        ...
        setExpandedLlmMessageId(null);
        expandedLlmTriggerRef.current = null;
    ...
    const expandedLlmMessage = llmMessages.find((message) => message.id === expandedLlmMessageId) || null;
    ...
    return (
        ...
        >
            ...
            >
              <aside className="wf-inspector" aria-label="节点配置" data-node-type={node.data.nodeType}>
                <header className="wf-node-editor-drag-handle">
                    ...
                    <div className="wf-inspector-actions">
                        ...
                        <button type="button" disabled={!llmSaveAllowed} onClick={onSave} title={llmSaveAllowed ? (node.data.savedAt && !node.data.isDirty ? `已保存 ${node.data.savedAt}` : '保存') : '请修正高级参数'} aria-label="保存"><Save size={15} /></button>
                                                    ...
                                                    {llmMessages.map((message, index) => {
                                                        ...
                                                        return (
                                                            <article className="wf-llm-message" key={message.id}>
                                                                ...
                                                                <div className="wf-llm-message-editor">
                                                                    <MarkdownPromptEditor
                                                                        ariaLabel={`${message.role} 消息内容`}
                                                                        value={message.content || ''}
                                                                        placeholder={LLM_MESSAGE_PLACEHOLDERS[message.role] || ''}
                                                                        error={error}
                                                                        theme={theme}
                                                                        onChange={(content) => updateLlmMessage(message.id, content)}
                                                                    />
                                                                    <button
                                                                        type="button"
                                                                        className="wf-llm-message-expand"
                                                                        onClick={(event) => {
                                                                            expandedLlmTriggerRef.current = event.currentTarget;
                                                                            setExpandedLlmMessageId(message.id);
                                                                        }}
                                                                        title={`放大编辑 ${message.role} 上下文`}
                                                                        aria-label={`放大编辑 ${message.role} 上下文`}
                                    ...
                                    <div className="wf-config-panel wf-output-variable-list">
                                        {startInputs.map((row, index) => (
                                            <div className="wf-output-variable-row wf-start-input-row" key={row.id}>
                                                <label data-start-field="name"><span>变量名</span><input aria-label={`START 变量名 ${index + 1}`} value={row.name} onChange={(event) => updateStartInput(row.id, {name: event.target.value})} /></label>
                                                <label data-start-field="value"><span>值</span><input aria-label={`START 变量值 ${index + 1}`} value={row.value} onChange={(event) => updateStartInput(row.id, {value: event.target.value})} placeholder={row.type === 'string' ? '输入文本' : '输入 JSON 值'} /></label>
                                                <label data-start-field="type"><span>类型</span><select aria-label={`START 变量类型 ${index + 1}`} value={row.type} onChange={(event) => updateStartInput(row.id, {type: event.target.value})}>{OUTPUT_VARIABLE_TYPES.map((type) => <option value={type} key={type}>{type}</option>)}</select></label>
                                                {index === 0 ? (
                            ...
                            {meta.executable && <section className="wf-config-section wf-editor-full-row">
                                ...
                                {showOutputVariables && (
                                    <>
                                        ...
                                        {mappingOpen && (
                                            <div className="wf-config-panel wf-output-variable-list" data-node-type={node.data.nodeType}>
                                                {outputVariables.map((row, index) => (
                                                    <div className="wf-output-variable-row" key={row.id}>
                                                        <label data-output-field="name"><span>变量名</span><input aria-label={`输出变量名 ${index + 1}`} value={row.name} onChange={(event) => updateOutputVariable(row.id, {name: event.target.value})} /></label>
                                                        <label data-output-field="source"><span>{isScript ? 'Python 变量' : '提取表达式'}</span><input aria-label={`输出变量来源 ${index + 1}`} value={row.value || ''} onChange={(event) => updateOutputVariable(row.id, {value: event.target.value})} /></label>
                                                        <label data-output-field="type"><span>类型</span><select aria-label={`输出变量类型 ${index + 1}`} value={row.type || 'string'} onChange={(event) => updateOutputVariable(row.id, {type: event.target.value})}>{OUTPUT_VARIABLE_TYPES.map((type) => <option value={type} key={type}>{type}</option>)}</select></label>
                                                        {index === 0 ? (
          ...
          {expandedLlmMessage && (
              <Dialog.Root open onOpenChange={(open) => !open && setExpandedLlmMessageId(null)}>
                  <Dialog.Portal>
                      <Dialog.Overlay className="wf-llm-editor-overlay" />
                      <Dialog.Content
                          className="wf-llm-editor-dialog"
                          aria-describedby={undefined}
                          onOpenAutoFocus={(event) => event.preventDefault()}
                          onCloseAutoFocus={(event) => {
                              event.preventDefault();
                              expandedLlmTriggerRef.current?.focus();
                          }}
                      >
                          <header>
                              <div>
                                  <Dialog.Title asChild><strong>编辑 {expandedLlmMessage.role} 上下文</strong></Dialog.Title>
                                  <span>{String(expandedLlmMessage.content || '').length} 字符</span>
                              </div>
                              <Dialog.Close asChild>
                                  <button type="button" title="关闭放大编辑" aria-label="关闭放大编辑"><X size={18} /></button>
                              </Dialog.Close>
                          </header>
                          <MarkdownPromptEditor
                              autoFocus
                              expanded
                              ariaLabel={`${expandedLlmMessage.role} 上下文放大编辑器`}
                              value={expandedLlmMessage.content || ''}
                              placeholder={LLM_MESSAGE_PLACEHOLDERS[expandedLlmMessage.role] || ''}
                              theme={theme}
                              onChange={(content) => updateLlmMessage(expandedLlmMessage.id, content)}
                          />
                      </Dialog.Content>
                  </Dialog.Portal>
              </Dialog.Root>
...
function WorkflowStudio({options}) {
    ...
    const [nodeLeaveRequest, setNodeLeaveRequest] = useState(null);
    const [nodeLeaveBusy, setNodeLeaveBusy] = useState(false);
    ...
    const nodeSavePromisesRef = useRef(new Map());
    const editorNodeBaselineRef = useRef(null);
    ...
    const persistDraft = useCallback(async ({forNodeRun = false, metadata = null, validateStructure = false} = {}) => {
        ...
        const nodesForSave = nodes.map(trimTrailingEmptyLlmNodeMessages);
        try {
            const saved = await options.onPersist({
                id: workflowIdRef.current,
                name,
                description,
                nodes: nodesForSave.map(serializableNode),
            ...
            if (!forNodeRun) {
                ...
                setNodes((current) => current.map((node) => {
                    const cleanedNode = trimTrailingEmptyLlmNodeMessages(node);
                    return {
                        ...cleanedNode,
                        data: {...cleanedNode.data, isDirty: false},
                    };
                }));
            }
            return saved.id;
    ...
    const addNodeAt = useCallback((type, position) => {
        if (['START', 'END'].includes(type) && nodes.some((node) => node.data.nodeType === type)) {
            closeMenus();
            return null;
        }
        recordHistory();
        ...
        closeMenus();
        return next;
    }, [closeMenus, nodes, recordHistory, setNodes]);
        ...
        try {
            ...
            source.addEventListener('error', (event) => {
                ...
                applyNodeTestSnapshot(nodeId, `failed-${nodeId}`, {
                    ...
                    inputs: context,
                    outputs: {},
                    error: {code: 'NODE_TEST_STREAM_ERROR', message, details: null},
                }, true);
    ...
    const loadNodeRuns = useCallback(async (id, activeWorkflowId = workflowId) => {
        ...
        try {
            const response = await fetch(`/api/workflows/${encodeURIComponent(activeWorkflowId)}/nodes/${encodeURIComponent(id)}/runs`, {
                headers: {accept: 'application/json'},
            });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(payload.detail || `HTTP ${response.status}`);
            const runs = (Array.isArray(payload.executions) ? payload.executions : [])
                .map(nodeExecutionHistoryRun);
    ...
    const saveNode = useCallback(async (id) => {
        const activeSave = nodeSavePromisesRef.current.get(id);
        if (activeSave) return activeSave;
        const savedAt = new Date().toLocaleTimeString('zh-CN', {hour12: false});
        const node = nodes.find((item) => item.id === id);
        if (!node) return false;
        const nodeForSave = trimTrailingEmptyLlmNodeMessages(node);
        const operation = (async () => {
            try {
                if (creationPromise.current) await creationPromise.current;
                if (typeof options.onPersistNode !== 'function') {
                    throw new Error('节点持久化入口不可用');
                }
                if (!workflowName.trim()) throw new Error('Workflow 名称不能为空');
                const saved = await options.onPersistNode({
                    id: workflowIdRef.current,
                    name: workflowName,
                    description: workflowDescription,
                    node: serializableNode(nodeForSave),
                });
                workflowIdRef.current = saved.id;
                nodeTestWorkflowIdRef.current = saved.id;
                setWorkflowId(saved.id);
                savedMetadata.current = {
                    name: saved.name ?? workflowName,
                    description: saved.description ?? workflowDescription,
                };
                metadataDraft.current = {...savedMetadata.current};
            } catch (error) {
            ...
            setNodes((current) => current.map((item) => {
                ...
                const cleanedItem = trimTrailingEmptyLlmNodeMessages(item);
                return {
                    ...cleanedItem,
                    data: {...cleanedItem.data, savedAt, isDirty: false},
                };
            }));
        ...
        nodeSavePromisesRef.current.set(id, operation);
        try {
            return await operation;
        } finally {
            if (nodeSavePromisesRef.current.get(id) === operation) {
                nodeSavePromisesRef.current.delete(id);
            }
        }
    }, [nodes, options, setNodes, workflowDescription, workflowName]);
    ...
    const applyWorkflowNodeRuns = useCallback((runs) => {
        ...
        setNodes((current) => current.map((node) => {
            ...
            return {...node, data: {...node.data, status: run.status, executionDurationMs: workflowNodeExecutionDuration(run, node.data.executionDurationMs), executionStartedAt: run.status === 'RUNNING' ? run.started_at : null, executionId: run.status === 'RUNNING' ? run.node_execution_id : null, runHistory: [historyRun].concat((node.data.runHistory || []).filter((item) => item.id !== run.node_execution_id)).slice(0, 10)}};
        }));
    }, [setNodes]);
    ...
    const runAll = useCallback(async () => {
        ...
        setNodes((current) => current.map(resetWorkflowNodeForRun));
        try {
    ...
    const finishInspectorExit = useCallback((request) => {
        const active = nodeTestSourcesRef.current.get(request.nodeId);
        if (active) {
            hiddenNodeTestsRef.current.add(request.nodeId);
            void interruptNodeTest(request.nodeId);
        } else {
            resetNodeTestDisplay(request.nodeId);
        }
        setEditorInitialTab(request.nextInitialTab || 'settings');
        setEditorNodeId(request.nextNodeId || null);
        setNodeLeaveRequest(null);
    }, [interruptNodeTest, resetNodeTestDisplay]);

    const requestInspectorExit = useCallback((request) => {
        const activeNode = nodes.find((node) => node.id === request.nodeId);
        if (activeNode?.data.isDirty) {
            setNodeLeaveRequest(request);
            return false;
        }
        finishInspectorExit(request);
        return true;
    }, [finishInspectorExit, nodes]);

    const openInspector = useCallback((nodeId, initialTab = 'settings') => {
        if (editorNodeId && editorNodeId !== nodeId) {
            return requestInspectorExit({
                nodeId: editorNodeId,
                nextNodeId: nodeId,
                nextInitialTab: initialTab,
            });
        }
        setEditorInitialTab(initialTab);
        setEditorNodeId(nodeId);
        return true;
    }, [editorNodeId, requestInspectorExit]);
    ...
    const decoratedNodes = useMemo(() => {
        ...
        const nowMs = workflowRunRef.current.startedAtMs + workflowElapsedMs;
        return nodes.map((node) => {
            const liveDurationMs = workflowNodeLiveDuration(node.data, nowMs);
            const cached = cache.get(node.id);
            if (cached && cached.node === node && cached.liveDurationMs === liveDurationMs && cached.onRun === requestNodeTest && cached.openInspector === openInspector) {
            ...
            const decorated = {
                ...
                data: {
                    ...node.data,
                    executionDurationMs: liveDurationMs,
                    onConfigure: () => void openInspector(node.id, 'settings'),
                    onRun: () => requestNodeTest(node.id),
                    onOpenLogs: () => void openInspector(node.id, 'logs'),
                },
            };
            cache.set(node.id, {node, decorated, liveDurationMs, onRun: requestNodeTest, openInspector});
            return decorated;
        });
    }, [nodes, openInspector, requestNodeTest, workflowElapsedMs]);

    const editorNode = nodes.find((node) => node.id === editorNodeId) || null;

    useEffect(() => {
        if (!editorNode) {
            editorNodeBaselineRef.current = null;
            return;
        }
        if (editorNodeBaselineRef.current?.id !== editorNode.id || !editorNode.data.isDirty) {
            editorNodeBaselineRef.current = serializableNode(editorNode);
        }
    }, [editorNode]);
    ...
    const save = useCallback(async () => {
        try {
            await persistDraft({validateStructure: true});
    ...
    const closeInspector = useCallback(() => {
        ...
        requestInspectorExit({nodeId: editorNodeId, nextNodeId: null, nextInitialTab: 'settings'});
    }, [editorNodeId, requestInspectorExit]);

    const discardNodeChangesAndLeave = useCallback(() => {
        const request = nodeLeaveRequest;
        if (!request) return true;
        const baseline = editorNodeBaselineRef.current;
        if (baseline?.id === request.nodeId) {
            setNodes((current) => current.map((node) => node.id === request.nodeId ? {
                ...node,
                data: {...node.data, ...cloneValue(baseline.data), isDirty: false},
            } : node));
        }
        finishInspectorExit(request);
        return true;
    }, [finishInspectorExit, nodeLeaveRequest, setNodes]);

    const saveNodeAndLeave = useCallback(async () => {
        const request = nodeLeaveRequest;
        if (!request) return true;
        setNodeLeaveBusy(true);
        try {
            const saved = await saveNode(request.nodeId);
            if (!saved) return false;
            finishInspectorExit(request);
            return true;
        } finally {
            setNodeLeaveBusy(false);
        }
    }, [finishInspectorExit, nodeLeaveRequest, saveNode]);
        ...
        <div className="workflow-studio-shell" tabIndex={0} aria-label="工作流画布" onKeyDown={handleKeyboard} onCopy={handleCopy} onPaste={handlePaste} onPointerDownCapture={handleMarqueeStart} onPointerMoveCapture={handleMarqueeMove} onPointerUpCapture={handleMarqueeEnd} onContextMenu={(event) => event.preventDefault()}>
            <header className="wf-studio-header">
                <div className="wf-header-left">
                    <button type="button" className="wf-icon-button" onClick={close} title="返回" aria-label="返回"><ArrowLeft size={18} /></button>
            ...
            <main className="wf-canvas-wrap" ref={canvasRef}>
                ...
                <ReactFlow
                    ...
                    onNodeDoubleClick={(event, node) => {
                        event.preventDefault();
                        closeMenus();
                        void openInspector(node.id, 'settings');
                    }}
                ...
                <ContextMenu
                    ...
                    unavailableSystemTypes={nodes
                        .filter((node) => ['START', 'END'].includes(node.data.nodeType))
                ...
                <Inspector
                    ...
                    onLoadRuns={() => editorNodeId && options.executionEnabled ? loadNodeRuns(editorNodeId) : []}
            ...
            <ConfirmDialog
                open={Boolean(nodeLeaveRequest)}
                title="保存节点修改？"
                confirmLabel="保存"
                secondaryLabel="不保存"
                busy={nodeLeaveBusy}
                onClose={() => setNodeLeaveRequest(null)}
                onSecondary={discardNodeChangesAndLeave}
                onConfirm={saveNodeAndLeave}
            >
                <p>当前节点还有未保存的修改。</p>
                <p className="execution-confirm-note">保存后继续离开，或放弃本次节点修改。选择取消可返回配置面板继续编辑。</p>
            </ConfirmDialog>
            <ConfirmDialog
